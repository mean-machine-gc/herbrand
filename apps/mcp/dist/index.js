...
import { z } from "zod";
import { parseSpecs, specLint, buildDecisionGraph, behaviorLint, createSpec, updateSpec, readSpec, listSpecs, readGraph, runPipeline, } from "@herbrand/core";
...
server.addTool({
    ...
    execute: async (args) => {
        ...
        const result = createSpec(projectDir, specData);
        return JSON.stringify(result, null, 2);
    },
});
server.addTool({
...
server.addTool({
    ...
    execute: async (args) => {
        ...
        if (!result)
            return "No decision graph available. Run build_decision_graph first.";
        return JSON.stringify(result, null, 2);
    },
});
server.addTool({
