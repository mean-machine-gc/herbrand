...
export function extractUserStories(graph: DecisionGraph): Record<string, UserStory> {
  ...
  for (const [name, spec] of Object.entries(specs)) {
    ...
    const outcomeEdge = edges.find(
      (e) => e.type === "outcome_flow" && e.from === intent
    );
    ...
    const given: UserStoryPrecondition[] = spec.rejects.map((tag) => {
    ...
    const shouldFailIf: UserStoryConstraint[] = [];
    if (outcomeSpec) {
    ...
    for (let i = 0; i < preconditionColumns.length; i++) {
      ...
      const constraints: Record<string, boolean> = {};
      for (const c of constraintColumns) constraints[c] = true; // N/A but marked true
    ...
    const views: UserStoryView[] = nodes
      .filter((n) => n.type === "view")
      .filter((n) =>
        edges.some((e) => e.from === n.id && e.to === intent && e.type === "view_to_intent")
      )
      .map((n) => ({
        id: n.id,
        infos: n.infos ?? [],
