import type { ParsedSpecs, DecisionGraph, GraphNode, GraphEdge } from "./types.js";

export function buildDecisionGraph(parsed: ParsedSpecs): DecisionGraph {
  ...
  const outcomeRejectSet = new Set<string>();
  ...
  const outcomeToRole: Record<string, string> = {};
  for (const [, spec] of Object.entries(specs)) {
    if (spec.type === "outcome") {
  ...
  // Build views
  type View = { id: string; infos: string[]; targetIntent: string; role: string | null };
  const views: View[] = [];
