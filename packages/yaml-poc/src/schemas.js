...
const infoEntry = (infoEnum) => z.object({
...
const successEntry = (infoEnum) => z.object({
...
const assertionEntry = (infoEnum) => z.object({
...
export function buildDecisionSchema(project) {
  const outcomesEnum = z.enum(project.outcomes).describe("Must be a declared outcome from project.hb.yaml");
  const intentsEnum = z.enum(project.intents).describe("Must be a declared intent from project.hb.yaml");
  const infoEnum = z.enum(project.info).describe("Must be a declared info unit from project.hb.yaml");
  const outcomeRejectsEnum = z.enum(project.outcomeRejects).describe("Must be a declared outcome reject from project.hb.yaml");
  const contextsEnum = z.enum(project.contexts).describe("Must be a declared context from project.hb.yaml");
  const modulesEnum = z.enum(project.modules).describe("Must be a declared module from project.hb.yaml");
  const aggregatesEnum = z.enum(project.aggregates).describe("Must be a declared aggregate from project.hb.yaml");

  const rejectionEnum = z.enum(project.outcomeRejects.map(r => `rejected:${r}`)).describe("Rejection event in rejected:tag format");
  ...
  // Intent decision — answers: what should happen?
  ...
  // Outcome decision — answers: what has happened?
  ...
  const outcomeDecisionSchema = z.object({
    ...
    shouldFailWith: z.record(outcomeRejectsEnum, infoEntry(infoEnum))
      .describe("Failure constraints — if any fails, a rejected:tag event is produced"),
    shouldSucceedWith: z.record(outcomesEnum, successEntry(infoEnum))
      .refine(
        (outcomes) => Object.keys(outcomes).length >= 1,
        "Must have at least one success outcome"
      )
      .refine(
        (outcomes) => Object.values(outcomes).some(o => o.condition === "always"),
        "At least one success outcome must have condition 'always'"
      )
      .describe("Success outcomes — at least one must have condition 'always' as the default path"),
    shouldAssert: z.record(outcomesEnum, z.array(assertionEntry(infoEnum)).min(1, "Each outcome must have at least one assertion"))
